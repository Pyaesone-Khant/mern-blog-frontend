...
const CropEasyModal = () => {
    ...
    return (
        <section>
            ...
            <Modal open={openModal} footer={null} centered={true} onCancel={() => setOpenModal(false)} width={400}>
                ...
                <div className={`my-5`}>
                    ...
                    <input type={"range"} id={"roomRange"} step={0.01} onChange={(e) => setZoom(e.target.value)} min={1}
                           max={3} value={zoom}
                           className={`w-full h-1 bg-gray-500 rounded-md appearance-none cursor-pointer range-sm `}/>
                </div>
                <div className={`flex items-center gap-5  `}>
                    <CustomBtn onClick={onCanceled}>
                        Cancel
                    </CustomBtn>
                    <CustomBtn onClick={handleCropImage}>
                        Crop & Upload
                    </CustomBtn>
                </div>
            </Modal>
