...
// apis
import {useGetSearchedDataQuery} from "@/features/auth/authApi.js";
    ...
    return <Form form={form} onFinish={onSearch}
        ...
        <Form.Item name={"keyword"} className={`mb-0 w-full`}>
            <Input placeholder={`Search Here . . .`}
                   className={"md:h-10 h-9 text-sm !w-full dark:!text-white dark:!placeholder-gray-400 !border-none !bg-transparent focus:!shadow-none px-1"}
                   onChange={onChange} autoComplete="off" onFocus={() => setIsSearching(true)}
                   onBlur={() => setIsSearching(false)}/>
        </Form.Item>


        <MdClear onClick={onClear}
                 className={cn(`px-0 py-0 absolute  top-1/2 transform -translate-y-1/2 duration-300 text-2xl cursor-pointer text-black/70 dark:text-white/70`, {
                     "right-2": !isInvalid,
                     "-right-20": isInvalid
                 })}/>
    </Form>
