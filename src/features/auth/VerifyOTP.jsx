...
const VerifyOTP = () => {
    ...
    return (
        <AuthComponentWrapper>
            <Form form={form} onFinish={onOtpVerify} className={`max-w-md w-full px-4`}>
                <div className={`text-center mb-8`}>
                    <h2 className={`form-tlt`}>Verify OTP</h2>
                    <p className={` dark:text-gray-300  text-gray-800`}> Please enter the verification code sent
                        to <span
                            className={`font-bold dark:text-white text-black`}> {newEmail ? newEmail : email} </span>
                    </p>
                </div>
                ...
                <div className="flex flex-col my-6 gap-1 md:gap-3 items-center text-c26 ">
                    <p className={"text-sm dark:text-gray-300  text-gray-800"}>Do not receive an OTP?</p>
                    <p className={`text-xl dark:text-gray-300  text-gray-800 ${isResent ? "block" : "hidden"} `}>
                        {" "}
                        {timer} s{" "}
                    </p>
                    <CustomBtn variant={"ghost"} onClick={onOtpResend} loading={isRequesting}
                               className={`text-base font-medium`} disabled={isResent}>
                        Resend OTP
                    </CustomBtn>
                </div>

                <CustomBtn htmlType={"submit"} className={`w-full`} loading={isVerifying}>
                    Submit
                </CustomBtn>
                <Link to={"/"}
