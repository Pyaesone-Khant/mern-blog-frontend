...
const ChangeNameModal = ({isUserAuth}) => {
    ...
    return (
        <section
            className={` ${!isUserAuth ? "hidden" : ""} flex flex-row items-center justify-between font-medium p-5 rounded-md bg-cBlue/10 dark:bg-darkTer/10`}>
            <h3 className="md:min-w-[120px] text-lg"> Password : </h3>
            <CustomBtn variant={"outline"} size={"sm"} onClick={() => setOpenModal(true)}>
                Change
            </CustomBtn>
            <Modal centered width={420} open={openModal} footer={null} closeIcon={false} className={`auth-modal`}>
                ...
                <Form form={form} onFinish={onNameChange} layout={"vertical"} className={`p-6 bg-cBlue/10`}>
                    <Form.Item label={<FormLabel label={"current password"}/>} hasFeedback={true} name={"password"}
                               rules={[
                                   {required: true, message: "Current password is required!"}
                               ]}>
                        <Input.Password placeholder={"Enter your current password"}/>
                    </Form.Item>
                    <Form.Item label={<FormLabel label={"password"}/>} hasFeedback={true} name={"newPassword"}
                    ...
                    <Form.Item label={<FormLabel label={"confirm password"}/>} hasFeedback={true}
                               name={"password_confirmation"} dependencies={["newPassword"]}
                    ...
                    <CustomBtn htmlType={"submit"} className={"mt-9 w-full"}>
                        Confirm
                    </CustomBtn>
                </Form>
            </Modal>
