...
const RecentlySavedBlogs = () => {
    ...
    const userSavedBlogs = savedBlogs
        ?.slice()
        .sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt))
        .slice(0, 4);

    const { data: randomBlogs, isLoading: isRBLoading } =
    ...
    return (
        <section className={`w-full max-w-[40%] lg:max-w-[30%]`}>
            ...
            <div className={`space-y-8`}>
                {isSBLoading || isRBLoading ? (
                    Array(4)
                        .fill(null)
                        .map((_, i) => <BlogLoader key={i} />)
                ) : displayedBlogs?.length ? (
                    displayedBlogs?.map((blog) => (
                        <SavedBlog blog={blog} key={blog?._id} />
                    ))
                ) : (
...
const SavedBlog = ({ blog }) => {
    ...
    return (
        <section className={`space-y-1`}>
            ...
            <Link
                to={`/${nameSlug}/${titleSlug}`}
                state={blogId}
                className={`block space-y-1 w-fit`}
            >
                <h2 className={`font-bold w-fit text-xl`}>{title}</h2>
                <p
                    className={`line-clamp-2 text-sm dark:text-gray-300 text-gray-700 font-medium`}
                >
                    {description}
                </p>
                <p
                    className={`text-xs dark:text-gray-400 text-gray-600 font-grm w-fit`}
                >
                    {date}{" "}
                </p>
            </Link>
